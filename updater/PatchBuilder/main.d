...
void main(string[] args)
{
	if(args.length < 3)
	{
		writeln("Usage : MD5builder.exe oldFolder newFolder");
		return;
	}
	...
	Entry[] entries1;
	foreach (string name; dirEntries(folder1, SpanMode.breadth))
	{
		if(name.isFile())
	...
	Entry[] entries2;
	foreach (string name; dirEntries(folder2, SpanMode.breadth))
	{
		if(name.isFile())
...
/*	if(Different.length == 0 && New.length == 0 && Deleted.length == 0)
	...
	foreach(Entry ent; Different)
	{
		copy(folder2~ "/" ~ ent.name, "output/" ~ ent.name);
		std.file.append("output/Changelog.txt", "* " ~ ent.name ~"\r\n");
	}
	foreach(Entry ent; New)
	{
		copy(folder2~ "/" ~ ent.name, "output/" ~ ent.name);
		std.file.append("output/Changelog.txt", "+ " ~ ent.name ~"\r\n");
	}
