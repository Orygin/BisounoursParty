<?php
	...
	$catId = intval($_GET['cat_id']);
        ...
        if($_GET['Add'] == 1)
        {
            ...
            {
                $db->query("INSERT INTO " . MYSQL_PREFIX . "devblog_comments (id, author, article, date, content) 
        ...
        if(!empty($delid))
        {
            ...
            {
                $req = $SQL->prepare("DELETE FROM " . MYSQL_PREFIX . "devblog_comments WHERE id = :id");
                $req->execute(array('id' => $delid));
            }
            else{
                $userid = intval($pun_user['id']);
                $req = $SQL->prepare("DELETE FROM " . MYSQL_PREFIX . "devblog_comments WHERE id=':id' AND author=':auth'");
                //$db->query("DELETE FROM bp_devblog_comments WHERE id ='" . $delid . "' AND author ='" . $userid . "'");
		...
		$comments = $SQL->query('SELECT          a.id AS commentId,
						...
						FROM ' . MYSQL_PREFIX . 'devblog_comments a
						INNER JOIN bpforum_users u
							ON a.author = u.id
	...
	if(!empty($catId)) {
		$return = $SQL->query('SELECT COUNT(*) as num FROM ' . MYSQL_PREFIX . 'devblog_articles WHERE lang = "' . USER_LANG . '" AND status = 1 AND categorie = ' . $catId . '');
        $count = $return->fetch();
		...
		$return = $SQL->query('SELECT COUNT(*) as num FROM ' . MYSQL_PREFIX . 'devblog_articles WHERE lang = "' . USER_LANG . '" AND status = 1');
        $count = $return->fetch();
	...
	$TPL->set($lang);
	$TPL->parse('blog/devblog_index.html');