...
#ifdef TF_CLIENT_DLL
		// PREITEMHACK: For now, just force our inventory to update whenever our achievements change
		InventoryManager()->UpdateLoadoutsFromSteam( true );
#endif
	}
	else if ( IsX360() )
	{
...
class NebuleuseAchievement2Steam : public IAchievement
{
public:
	...
	bool IsAchieved() { return Count>=Goal; }
	int GetPointValue() { return 0; }
	bool ShouldSaveWithGame() { return false; }
	bool ShouldHideUntilAchieved() { return false; }

	NebuleuseAchievement2Steam(int index, const char* lname, int flag, int lgoal, int lcount)
	{
		AchievementID = index;
		Name = lname;
		flags = flag;
...
{
/*#ifdef CLIENT_DLL
	AchievementData ach = g_Nebuleuse->GetAchievementData(index);
	NebuleuseAchievement2Steam *Achievement = new NebuleuseAchievement2Steam(index, ach.Name, 0, ach.ProgressMax, ach.Progress);
	return (IAchievement*)Achievement;
#endif*/
...
{
/**#ifdef CLIENT_DLL
	return g_Nebuleuse->GetAchievementCount();
#endif*/
...
#ifdef TF_CLIENT_DLL
	InventoryManager()->UpdateLoadoutsFromSteam();
#endif
}
