...
static ConVar cl_disable_radio("cl_disable_radio", "0", FCVAR_ARCHIVE);
...
#if defined( CLIENT_DLL )
		if(!cl_disable_radio.GetBool() && !Q_stristr(params.soundname, "bisounoursam/"))
...
{
	if(cl_disable_radio.GetBool() && Q_stristr(samp, "bisounoursam/"))
		return;
...
#else
			if(!cl_disable_radio.GetBool() && !Q_stristr(name, "bisounoursam/"))
