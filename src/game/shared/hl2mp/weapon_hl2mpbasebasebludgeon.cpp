...
//	AddViewKick();
	...
	if ( pHitEntity != NULL )
	{
		...
		//BP pour que les chocs type DMG_CLUB interagissent avec les props physiques
		CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
		if(pPlayer == NULL)
			return;

		Vector force;
		AngleVectors(pPlayer->GetAbsAngles(), &force);

		//CTakeDamageInfo info( GetOwner(), GetOwner(), Vector (100,100,100), BaseClass::GetTouchTrace().endpos, 10000.0f, GetDamageForActivity( nHitActivity ), DMG_CLUB, NULL);
		CTakeDamageInfo info( GetOwner(), GetOwner(), vec3_origin, vec3_origin, GetDamageForActivity( nHitActivity ), DMG_CLUB, NULL); 
