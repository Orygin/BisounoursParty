...
//  CWeaponCarebearStare class
//----------------------------------------------------------------------------------------------------------------------------------------------------------




IMPLEMENT_NETWORKCLASS_ALIASED( WeaponCarebearStare, DT_WeaponCarebearStare )

BEGIN_NETWORK_TABLE( CWeaponCarebearStare, DT_WeaponCarebearStare )
#ifdef CLIENT_DLL
...
#ifdef CLIENT_DLL
BEGIN_PREDICTION_DATA( CWeaponCarebearStare )
...
LINK_ENTITY_TO_CLASS( weapon_carebearstare, CWeaponCarebearStare );
PRECACHE_WEAPON_REGISTER( weapon_carebearstare );

acttable_t	CWeaponCarebearStare::m_acttable[] = 
{
	...
	{ ACT_MP_SWIM,						ACT_HL2MP_IDLE_PHYSGUN,					false },
};

IMPLEMENT_ACTTABLE(CWeaponCarebearStare);
...
CWeaponCarebearStare::CWeaponCarebearStare( void )
{
	m_bOpen					= false;
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::Precache( void )
{
	...
	PrecacheScriptSound( "Gameplay.Forts.Freeze" );
	PrecacheScriptSound( "Gameplay.Forts.UnFreeze" );
...
// Purpose: Restore
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::OnRestore()
{
	BaseClass::OnRestore();
	m_grabController.OnRestore();
...
// On Remove
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::UpdateOnRemove(void)
{
	DestroyEffects( );
...
#ifdef CLIENT_DLL
void CWeaponCarebearStare::OnDataChanged( DataUpdateType_t type )
{
...
// Sprite scale factor 
//-----------------------------------------------------------------------------
inline float CWeaponCarebearStare::SpriteScaleFactor() 
{
	return 1.0f;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponCarebearStare::Deploy( void )
{
	CloseElements();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::SetViewModel( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::ForceDrop( void )
{
	CloseElements();
	DetachObject();
	StopEffects();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponCarebearStare::DropIfEntityHeld( CBaseEntity *pTarget )
{
	if ( pTarget == NULL )
		return false;
...
void CWeaponCarebearStare::Drop( const Vector &vecVelocity )
{
	ForceDrop();
...
bool CWeaponCarebearStare::CanHolster( void ) 
{ 
...
bool CWeaponCarebearStare::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	//Don't holster this weapon if we're holding onto something
	if ( m_bActive )
		return false;
...
void CWeaponCarebearStare::DryFire( void )
{
...
void CWeaponCarebearStare::PrimaryFireEffect( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );	
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::PuntNonVPhysics( CBaseEntity *pEntity, const Vector &forward, trace_t &tr )
{
...
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::Physgun_OnPhysGunPickup( CBaseEntity *pEntity, CBasePlayer *pOwner, PhysGunPickup_t reason )
{
	// If the target is debris, convert it to non-debris
...
// Punt vphysics
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Input  : - 
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::ApplyVelocityBasedForce( CBaseEntity *pEntity, const Vector &forward )
{
#ifndef CLIENT_DLL
...
float CWeaponCarebearStare::TraceLength()
{
...
void CWeaponCarebearStare::PrimaryAttack( void )
{
...
void CWeaponCarebearStare::SecondaryAttack( void )
{
#ifndef CLIENT_DLL
...
void CWeaponCarebearStare::WeaponIdle( void )
{
...
bool CWeaponCarebearStare::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
...
CWeaponCarebearStare::FindObjectResult_t CWeaponCarebearStare::FindObject( void )
{
...
CBaseEntity *CWeaponCarebearStare::FindObjectInCone( const Vector &vecOrigin, const Vector &vecDir, float flCone )
{
...
void CWeaponCarebearStare::UpdateObject( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	Assert( pPlayer );
...
void CWeaponCarebearStare::DetachObject( bool playSound, bool wasLaunched )
{
#ifndef CLIENT_DLL
	if ( m_bActive == false )
		return;
...
#ifdef CLIENT_DLL
void CWeaponCarebearStare::ManagePredictedObject( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::UpdateElementPosition( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
void CWeaponCarebearStare::ClientThink( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ConVar mp_forts_freeze_range( "mp_forts_freeze_range", "320",	FCVAR_REPLICATED | FCVAR_NOTIFY, "", true, 64, false, 0);
ConVar mp_forts_freeze_delay( "mp_forts_freeze_delay", "0.5",		FCVAR_REPLICATED | FCVAR_NOTIFY, "", true, 0.1, false, 0);

void CWeaponCarebearStare::ItemPreFrame()
{
	...
	if(HL2MPRules()->GetGameType() == GAME_FORTS && HL2MPRules()->IsFortsBuildPhase())
	{
		...
		{
			if(gpGlobals->curtime < m_flLastFreezeTime + mp_forts_freeze_delay.GetFloat())
				return;
			...
			if( m_grabController.GetAttached() )
			{
				...
				if(!FClassnameIs(pObject, "prop_physics"))
					return;
				...
				if(pPhysicsObject->IsMoveable())
				{
					...
					EmitSound( "Gameplay.Forts.Freeze" );
				}
			}
			else
			{
				...
				float flPuntDistance = mp_forts_freeze_range.GetFloat();
				VectorMA( start, flPuntDistance, forward, end );
				...
				if(pPhysicsObject->IsMoveable())
					...
					EmitSound( "Gameplay.Forts.UnFreeze" );

				pPhysicsObject->Wake();
				pPhysicsObject->EnableMotion(!pPhysicsObject->IsMoveable());
...
void CWeaponCarebearStare::CheckForTarget( void )
{
#ifndef CLIENT_DLL
...
void CWeaponCarebearStare::DoEffectIdle( void )
{
#ifdef CLIENT_DLL
...
void CWeaponCarebearStare::ItemPostFrame()
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner == NULL )
	{
...
void CWeaponCarebearStare::LaunchObject( const Vector &vecDir, float flForce )
{
	CBaseEntity *pObject = m_grabController.GetAttached();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponCarebearStare::CanPickupObject( CBaseEntity *pTarget )
{
#ifndef CLIENT_DLL
	if ( pTarget == NULL )
		return false;
...
void CWeaponCarebearStare::OpenElements( void )
{
	if ( m_bOpen )
		return;
...
void CWeaponCarebearStare::CloseElements( void )
{
	if ( m_bOpen == false )
		return;
...
float CWeaponCarebearStare::GetLoadPercentage( void )
{
...
CSoundPatch *CWeaponCarebearStare::GetMotorSound( void )
...
void CWeaponCarebearStare::StopLoopingSounds()
{
	if ( m_sndMotor != NULL )
	{
...
void CWeaponCarebearStare::DestroyEffects( void )
{
#ifdef CLIENT_DLL
...
void CWeaponCarebearStare::StopEffects( bool stopSound )
{
...
#ifdef CLIENT_DLL
void CWeaponCarebearStare::ThirdPersonSwitch( bool bThirdPerson )
{
...
void CWeaponCarebearStare::StartEffects( void )
{
#ifdef CLIENT_DLL
...
void CWeaponCarebearStare::DoEffectClosed( void )
{
...
void CWeaponCarebearStare::DoEffectReady( void )
{
...
void CWeaponCarebearStare::DoEffectHolding( void )
{
...
void CWeaponCarebearStare::DoEffectLaunch( Vector *pos )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner == NULL )
		return;
...
void CWeaponCarebearStare::DoEffectNone( void )
{
#ifdef CLIENT_DLL
...
void CWeaponCarebearStare::DoEffect( int effectType, Vector *pos )
{
...
const char *CWeaponCarebearStare::GetShootSound( int iIndex ) const
{
...
//			effect parameter
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::GetEffectParameters( EffectType_t effectID, color32 &color, float &scale, IMaterial **pMaterial, Vector &vecAttachment )
{
...
//-----------------------------------------------------------------------------
bool CWeaponCarebearStare::IsEffectVisible( EffectType_t effectID )
{
...
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::DrawEffectSprite( EffectType_t effectID )
{
	color32 color;
	float scale;
	IMaterial *pMaterial;
...
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::DrawEffects( void )
{
...
//-----------------------------------------------------------------------------
int CWeaponCarebearStare::DrawModel( int flags )
{
...
//-----------------------------------------------------------------------------
void CWeaponCarebearStare::ViewModelDrawn( C_BaseViewModel *pBaseViewModel )
{
	// Render our effects
	DrawEffects();
...
bool CWeaponCarebearStare::IsTransparent( void )
{
	return true;
}
...
void CWeaponCarebearStare::NotifyShouldTransmit( ShouldTransmitState_t state )
{
...
void PhysCannonForceDrop( CBaseCombatWeapon *pActiveWeapon, CBaseEntity *pOnlyIfHoldingThis )
{
	CWeaponCarebearStare *pCannon = dynamic_cast<CWeaponCarebearStare *>(pActiveWeapon);
	if ( pCannon )
	{
