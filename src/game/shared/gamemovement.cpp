...
// Purpose: 
//-----------------------------------------------------------------------------
//ConVar mp_bunnyhop("mp_bunnyhop",		"1", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_CHEAT , "Enable/Disable Bunny Hopping", true, 0, true, 1);
ConVar mp_jumpfactor("mp_jumpfactor",	"1.5", FCVAR_REPLICATED|FCVAR_CHEAT, "Jump Height Factor",		true, 0, false, 0);
//ConVar mp_pogostick("mp_pogostick",		"1", FCVAR_REPLICATED|FCVAR_NOTIFY, "Enable/Disable pogostick",		true, 0, true, 1);
//void CGameMovement::SetPogoStick(bool b)
//{
//	if(b)
//		mp_pogostick.SetValue(1);
//	else
//		mp_pogostick.SetValue(0);
//}
...
//#ifndef CLIENT_DLL // Client doesn't know about this
	if(mv->m_nOldButtons & IN_JUMP)
	{
		if(HL2MPRules()->GetForcePogo() == FORCE_OFF)
			return false;

		if(HL2MPRules()->GetForcePogo() != FORCE_ON)
			return false;
	}
//#endif

	if (  (  player->GetFlags() & FL_DUCKING ) )// (  player->m_Local.m_bDucking ) ||
		return false; //BP Pas de Duck puis saut
	...
	if ( (  player->m_Local.m_bDucking ) || (  player->GetFlags() & FL_DUCKING ) )
	{
		...
		// v^2 = g * g * 2.0 * 45 / g
		// v = sqrt( g * 2.0 * 45 )
		mv->m_vecVelocity[2] = flGroundFactor * flMul * mp_jumpfactor.GetFloat();  // 2 * gravity * height
	}
	else
		mv->m_vecVelocity[2] += flGroundFactor * flMul * mp_jumpfactor.GetFloat();  // 2 * gravity * height

	//BP bunny hop
	if (HL2MPRules()->GetForceBunny() != FORCE_OFF)
	{
		Vector vecForward;
		...
		if ( !player->m_Local.m_bDucked )
			for ( int iAxis = 0; iAxis < 2 ; ++iAxis )
				vecForward[iAxis] *= ( mv->m_flForwardMove * 0.55f );
		else
			for ( int iAxis = 0; iAxis < 2 ; ++iAxis )
	...
	//BP Son de saut
#ifndef CLIENT_DLL
	if(player->m_flJumpSoundAvoidTime > gpGlobals->curtime)
		return true;

	CRecipientFilter filter;
	filter.AddRecipientsByPAS( player->GetAbsOrigin() );
	CBaseEntity::EmitSound( filter, player->entindex(), "Player.Jump" );
#else
/*	if(player->m_flJumpSoundAvoidTime > gpGlobals->curtime)
		return true;
...
ConVar sv_autovocal_minspeed("sv_autovocal_minspeed", "980", FCVAR_REPLICATED | FCVAR_CHEAT);
...
#ifndef CLIENT_DLL
	const Vector vel = player->GetAbsVelocity();
	if (vel.IsLengthGreaterThan(sv_autovocal_minspeed.GetFloat()) && vel.z > 0)
		player->PlayAutovocal(HIGHSPEED);
	if (vel.IsLengthGreaterThan(1337) && vel.z > 0) // Atteignable facilement
		ToHL2MPPlayer(player)->EarnAchievement("Highspeed");
#endif
}
...
void CGameMovement::PlayerRoughLandingEffects( float fvol )
{
	if ( fvol > 0.0 )
	{
		...
		//player->PlayStepSound( (Vector &)mv->GetAbsOrigin(), player->m_pSurfaceData, fvol, true );
			...
			CBaseEntity::EmitSound( filter, player->entindex(), "Player.Land" );
		#endif
