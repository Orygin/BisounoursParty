...
ConVar mp_chattime(
		...
		true, 1,
		true, 120 );
ConVar mp_falldamageratio( "mp_falldamageratio", "0.625", FCVAR_NOTIFY|FCVAR_ARCHIVE, "Falldamage value will be multiplicated by this");
...
bool CMultiplayRules::Init()
{
	...
	CBaseCombatWeapon *CMultiplayRules::GetNextBestWeapon( CBaseCombatCharacter *pPlayer, CBaseCombatWeapon *pCurrentWeapon )
	{
		...
		if(ToBasePlayer(pPlayer)->Weapon_GetLast() && ToBasePlayer(pPlayer)->Weapon_GetLast()->HasAnyAmmo())
			return ToBasePlayer(pPlayer)->Weapon_GetLast();
	...
	{
		int iFallDamage = HL2MPRules()->GetForceFallDamage();

		switch ( iFallDamage )
		{
		case 1://progressive
			pPlayer->m_Local.m_flFallVelocity -= PLAYER_MAX_SAFE_FALL_SPEED;
			return pPlayer->m_Local.m_flFallVelocity * DAMAGE_FOR_FALL_SPEED * mp_falldamageratio.GetFloat();
			break;
		case 0:
		//case 0:// fixed
			return 10;
			break;
		default: //BP Pas de dégats
			return 0;
			break;
		}
	} 
	...
	ConVar mp_pushfrag_delay("mp_pushfrag_delay", "4", FCVAR_REPLICATED | FCVAR_CHEAT);
	...
	*/
	...
	ConVar  mapcyclerandom( "mapcyclerandom", "1", FCVAR_NONE, "Randomize mapcycle");
	void CMultiplayRules::GetNextLevelName( char *pszNextMap, int bufsize, bool bRandom /* = false */ )
	{
		...
		if ( bRandom || mapcyclerandom.GetBool() )
		{
