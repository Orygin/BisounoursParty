...
//BP génère un effet de recul en poussant le joueur vers l'arrière
void CBaseCombatWeapon::AddRecoil(float maxX, float maxY, float maxZ, float multVelocity)
{
	...
	forward.y *= maxY;
	forward.z *= maxZ;
	pPlayer->SetAbsVelocity((pPlayer->GetAbsVelocity() * multVelocity) + forward);
}
...
void CBaseCombatWeapon::Precache( void )
{
	...
	else
	{
		...
		DevWarning( "Error reading weapon data file for: %s\n", GetClassname() );
...
void CBaseCombatWeapon::DefaultTouch( CBaseEntity *pOther )
{
	...
	if( ( m_NextPickAllowedPlayer == pOther ) && ( m_NextPickAllowed > gpGlobals->curtime ) )
		return;
...
bool CBaseCombatWeapon::DefaultDeploy( char *szViewModel, char *szWeaponModel, int iActivity, char *szAnimExt )
{
	...
	if ( pOwner )
	{
		...
		pOwner->SetNextAttack( gpGlobals->curtime + 0.2f );
	}
...
void CBaseCombatWeapon::PrimaryAttack( void )
{
	...
	//BP on vire pour laisser la place au muzzleflash maison
