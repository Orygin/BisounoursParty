...
#define ZOMBIE_REFIRE_TIME 0.8f
...
HeadcrabRelease_t CNPC_BaseZombie::ShouldReleaseHeadcrab( const CTakeDamageInfo &info, float flDamageThreshold )
{
	...
	return RELEASE_NO; //BP pas de headcrab!
}
...
int CNPC_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	//float flDamageThreshold = min( 1, info.GetDamage() / m_iMaxHealth ); BP on enlève 
...
extern ConVar sk_zombie_bisou_health;
...
CBaseEntity *CNPC_BaseZombie::ClawAttack( float flDist, int iDamage, QAngle qaViewPunch, Vector vecVelocityPunch, int BloodOrigin  )
{
	...
	{
		trace_t	tr;
		AI_TraceHull( WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), -Vector(8,8,8), Vector(8,8,16), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr );
	...
	if ( pHurt )
	{
		...
		if ( pPlayer != NULL && !(pPlayer->GetFlags() & FL_GODMODE ) )
		{
			...
			if(FClassnameIs( this, "npc_zombie_bisou")){
				int iBonusHealth = sk_zombie_bisou_health.GetInt() * RandomFloat(0.15f, 0.20f);
				m_iHealth = m_iHealth + iBonusHealth <= sk_zombie_bisou_health.GetInt() ? m_iHealth + iBonusHealth : sk_zombie_bisou_health.GetInt();
			}
			else if(FClassnameIs( this, "npc_zombie")){
				int iBonusHealth = sk_zombie_health.GetInt() * RandomFloat(0.15f, 0.20f);
				m_iHealth = m_iHealth + iBonusHealth <= sk_zombie_health.GetInt() ? m_iHealth + iBonusHealth : sk_zombie_health.GetInt();
			}
		}
	...
	case TASK_ZOMBIE_RELEASE_HEADCRAB:
		{
			...
			//ReleaseHeadcrab( EyePosition(), vecVelocity, true, true ); //Toujours pas de headcrab dans BP!
			TaskComplete();
		}
		break;
...
void CNPC_BaseZombie::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if(FClassnameIs(this, "npc_gingerbread"))
	{
		if(BPRandom::GetInt(0,8) == 0)
			pPlayer->PlayAutovocal(NPCKZOMBIE);
		if(HL2MPRules()->GetGameType() == GAME_COOP)
			pPlayer->AddPoints(3,false);
	}
	else if(FClassnameIs(this, "npc_zombie_bisou"))
	{
		if(BPRandom::GetInt(0,10) == 0)
			pPlayer->PlayAutovocal(NPCKBISOUZOMBIE);
		if(HL2MPRules()->GetGameType() == GAME_COOP)
			pPlayer->AddPoints(2,false);
	}
	else if(FClassnameIs(this, "npc_pedobear"))
	{
		if(BPRandom::GetInt(0,2) == 0)
			pPlayer->PlayAutovocal(NPCKPEDOBEAR);
		if(HL2MPRules()->GetGameType() == GAME_COOP)
			pPlayer->AddPoints(11,false);
	}
	else if(FClassnameIs(this, "npc_fastzombie"))
	{
		if(HL2MPRules()->GetGameType() == GAME_COOP)
...
{
	//BP Pas de headcrab dans BP, on dégage tout ça
	/*CAI_BaseNPC		*pCrab;
