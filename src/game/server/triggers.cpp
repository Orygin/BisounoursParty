...
void CTriggerTeleport::Touch( CBaseEntity *pOther )
{
	...
	if (!pentLandmark && !HasSpawnFlags(SF_TELEPORT_PRESERVE_ANGLES) )
	{
		...
		pVelocity = &vecZero;
#else
		Vector *pVelocity = new Vector();
		const float flatVel = pOther->GetAbsVelocity().Length();
		
		AngleVectors(pentTarget->GetAbsAngles(), pVelocity, NULL, NULL);

		VectorNormalize(*pVelocity);
		*pVelocity *= flatVel;
	
		pOther->Teleport( &tmp, &pentTarget->GetAbsAngles(), pVelocity );
		delete pVelocity;
#endif
	}
	else
