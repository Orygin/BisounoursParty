...
public:
	...
	void SetSkinNumberFromCvar(void);
	void UpdateSkin(void);
	void SetSkinNumberFromTeam(void);
	int GetRandomSkinNumber(void);
	...
	virtual void			Respawn(bool killPlayer);
	virtual void			Teleport(const Vector *newPosition, const QAngle *newAngles, const Vector *newVelocity);
	void					Telefrag(const Vector *position);
	...
	//BP Taunt
	void Taunt(void);
	bool m_bMarkedBazookaMissileDeletion;
...
public:
	...
	int		ArmorValue() const		{ return m_iArmorValue; }
	...
	void	LockSpeed(bool bBool){m_bLockSpeed = bBool;}
	void	SetMaxSpeed( float flMaxSpeed ) { if(!m_bLockSpeed) m_flMaxspeed = flMaxSpeed; }
...
protected:
	...
	float					m_flNextRespawn; //BP délai avant le respawn
...
private:
	...
	CNetworkVarForDerived( int, m_iArmorValue );
	float					m_AirFinished;
...
public:
	CNetworkVar( float, m_flJumpSoundAvoidTime );
	int						m_iFragCount;
	int						m_nAutovocalAtRespawn;
private:
	float					m_flLastAutovocal;
	float					m_flNextDelayedAutovocal;
	int						m_nDelayedAutovocal;
	bool					m_bIsFirstSpawn;
	float					m_flLastSpawnTime;
	float					m_flLastUpdatePositionsListTime;
	CUtlVector< Vector >	m_LastPositionsList;
...
public:
	...
	float					m_flLastDamageTime; //BP Dernière fois qu'il s'est pris un shoot
	CBasePlayer				*m_pLastAttacker; //BP Le dernier player qui l'a shooté
