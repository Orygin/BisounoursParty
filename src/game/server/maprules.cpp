...
BEGIN_DATADESC( CGameScoreTeam )
	DEFINE_INPUTFUNC( FIELD_VOID, "ApplyScore", InputApplyScore ),
	DEFINE_KEYFIELD( m_iPoints, FIELD_INTEGER, "points" ),
	DEFINE_KEYFIELD( m_iTeam, FIELD_INTEGER, "team" ),
END_DATADESC()

void CGameScoreTeam::InputApplyScore( inputdata_t &inputdata )
{
	if(m_iTeam == TEAM_PINK)
		g_Teams[TEAM_PINK]->AddScore(m_iPoints);
	else if(m_iTeam == TEAM_GREEN)
		g_Teams[TEAM_GREEN]->AddScore(m_iPoints);
	else
		Warning("game_score_team : Invalid team ID\n");
}
