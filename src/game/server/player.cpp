...
//	DEFINE_CUSTOM_FIELD( m_Activity, ActivityDataOps() ),
	...
	// from edict_t
	DEFINE_FIELD( m_iArmorValue, FIELD_INTEGER ),
...
CBasePlayer::CBasePlayer( )
{
	...
	m_flJumpSoundAvoidTime = 0;
	m_iFragCount = 0;
	m_bIsFirstSpawn = true;
	m_nAutovocalAtRespawn = AUTOVOCAL_NONE;
	m_nDelayedAutovocal = AUTOVOCAL_NONE;
	m_flNextDelayedAutovocal = 0.0f;
	m_flLastAutovocal = 0.0f;
	m_flLastSpawnTime = 0.0f;
	m_flLastUpdatePositionsListTime = 0.0f;
	m_bMarkedBazookaMissileDeletion = false;
	UpdateSkin();
}
...
void CBasePlayer::UpdateOnRemove( void )
{
	...
	Weapon_Drop(GetActiveWeapon(), &Vector(0,0,0), &Vector(0,0,0));
...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	// Armor. 
	if(m_iArmorValue && !(info.GetDamageType() & (DMG_FALL | DMG_DROWN | DMG_POISON | DMG_RADIATION)) ) // armor doesn't protect against fall or drown damage!
	{
		//BP
		if(info.GetDamageType() & DMG_BULLET)
		{
			flArmorRatio = 0.8f; //0.8
			flHealthRatio = 0.35f; //0.35
		}
		else if(info.GetDamageType() & DMG_BLAST)
		{
			flArmorRatio = 0.6f; //0.6
			flHealthRatio = 0.45f; //0.45
		}
		else if(info.GetDamageType() & DMG_SLASH)
		{
			flArmorRatio = 0.8f; //0.8
			flHealthRatio = 0.05f; //0.05

		}
		else if(info.GetDamageType() & DMG_CLUB)
		{
			flArmorRatio = 0.65f; //0.65
			flHealthRatio = 0.25f; //0.25

			EmitSound("Weapon_Melee.HitPlayer");
		}
		else
		{
				flArmorRatio = 0.2f; //0.2f
				flHealthRatio = 1.0f; //1.0f
		}

		float flArmorDamage = info.GetDamage() * flArmorRatio;
		float flHealthDamage = info.GetDamage() * flHealthRatio;

		flArmorDamage = clamp(flArmorDamage, 1, 200);

		// Does this use more armor than we have?
		if (flArmorDamage > m_iArmorValue)
		{
			flHealthDamage += flArmorDamage - m_iArmorValue;
			m_DmgSave = m_iArmorValue;
			m_iArmorValue = 0;
		}
		else
		{
			m_iArmorValue -= flArmorDamage;
			m_DmgSave = flArmorDamage;
		}
		
		info.SetDamage( flHealthDamage );
	}
...
void CBasePlayer::OnDamagedByExplosion( const CTakeDamageInfo &info )
{
	...
	ConVar *mp_jumpfactor = cvar->FindVar( "mp_jumpfactor" );

	//ApplyLocalVelocityImpulse(Vector (0,0, 0));
...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if(attacker->IsPlayer()){
		m_pLastAttacker = ToBasePlayer(attacker);
	...
	if ( info.GetInflictor() && (GetMoveType() == MOVETYPE_WALK) && ( !attacker->IsSolidFlagSet(FSOLID_TRIGGER)) )
	{
...
ConVar sv_respawntimemultiplier("sv_respawntimemultiplier", "0.25", FCVAR_NOTIFY);
...
void CBasePlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if(HL2MPRules()->GetGameType() == GAME_TDM)
		AddPointsToTeam(-1, true);

	//BP Calcule le délai avant le respawn
	int iTeammatesNumber = HL2MPRules()->IsTeamplay() ? g_Teams[GetTeamNumber()]->GetNumPlayers() : 5;//countPlayers();
	m_flNextRespawn = gpGlobals->curtime + iTeammatesNumber * sv_respawntimemultiplier.GetFloat();
	//UpdateSkin();
...
void CBasePlayer::Event_Dying()
{
	...
	if(HL2MPRules()->GetGameType() == GAME_FORTS)
	{
		if(GetTeamNumber() == TEAM_PINK)
		{
			...
			{
				if(HL2MPRules()->m_apFortsInfosPink[i] == this){
					HL2MPRules()->m_apFortsInfosPink[i] = NULL;
					IGameEvent *event = gameeventmanager->CreateEvent( "forts_event" );
					if ( event )
					{
						event->SetInt( "userid", this->GetUserID() );
						event->SetInt( "team", TEAM_GREEN );
						event->SetInt( "action", 2 );
						gameeventmanager->FireEvent( event );
					}
				}
			}
		...
		else
		{
			...
			for(int i = 0; i < FORTS_MAX_INFOS; i++)
			{
				if(HL2MPRules()->m_apFortsInfosGreen[i] == this){
					HL2MPRules()->m_apFortsInfosGreen[i] = NULL;
					IGameEvent *event = gameeventmanager->CreateEvent( "forts_event" );
					if ( event )
					{
						event->SetInt( "userid", this->GetUserID() );
						event->SetInt( "team", TEAM_PINK );
						event->SetInt( "action", 2 );
						gameeventmanager->FireEvent( event );
					}
				}
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	//BP Quand l'animation de mort est terminée on passe en mode spectateur (s'il y a plusieurs joueurs sur le serveur)
	if ( g_pGameRules->IsMultiplayer() && ( gpGlobals->curtime > (m_flDeathTime + DEATH_ANIMATION_TIME + 2) ) && !IsObserver() )
		if((HL2MPRules()->IsTeamplay() && g_Teams[TEAM_GREEN]->GetNumPlayers() + g_Teams[TEAM_PINK]->GetNumPlayers() > 1) || (!HL2MPRules()->IsTeamplay() && g_Teams[TEAM_UNASSIGNED]->GetNumPlayers() > 1))
			StartObserverMode( OBS_MODE_CHASE );
		else
			StartObserverMode( OBS_MODE_DEATHCAM );

	if (m_lifeState == LIFE_DEAD && g_pGameRules->FPlayerCanRespawn( this ))
	{
	...
	//BP Si le délai de respawn est passé on... Respawne!
...
bool CBasePlayer::IsValidObserverTarget(CBaseEntity * target)
{
	...
	//BP empêche à un spectateur de suivre un autre spectateur
...
void CBasePlayer::AddPoints( int score, bool bAllowNegativeScore )
{
	...
	if(HL2MPRules()->GetGameType() == 3) //forts
	{
		switch(cvar->FindVar( "mp_co_difficulty" )->GetInt())
		{
		case 1: //easy
			score += score;
			break;
...
{
	//BP empêche les spectateur d'utiliser la commande "utiliser"
	if (GetTeamNumber() == TEAM_SPECTATOR)
		ucmd->buttons &= ~IN_USE;
...
ConVar sv_autovocal_geolocalisation_distance("sv_autovocal_geolocalisation_distance", "512", FCVAR_CHEAT | FCVAR_REPLICATED, "If the players doesn't move further than x units whithin 20secs, play an autovocal");
...
void CBasePlayer::PostThink()
{
	...
	//BP
	if((gpGlobals->curtime > m_flNextDelayedAutovocal) && (m_nDelayedAutovocal != AUTOVOCAL_NONE))
			PlayAutovocal(m_nDelayedAutovocal);

	//BP Mise à jour de la liste des 20 dernières positions du player ces 20 dernières secondes
	if(gpGlobals->curtime > m_flLastUpdatePositionsListTime + 1.0f)
	{
		if(!(HL2MPRules()->GetGameType() == GAME_FORTS && HL2MPRules()->IsFortsBuildPhase()))
		{
			if(m_LastPositionsList.Count() == 20)
				m_LastPositionsList.Remove(19); // Pas plus de 20 positions, alors on Pop la dernière

			m_LastPositionsList.AddToHead(GetAbsOrigin());
			m_flLastUpdatePositionsListTime = gpGlobals->curtime;

			if(m_LastPositionsList.Count() == 20) // Ça doit faire au moins vings secondes que le player stagne
			{
				Vector vecMin, vecMax, vecBox;

				// On initialise d'abord, sinon ça fail, question de logique (min max avec des v0 toussah)
				vecMin = m_LastPositionsList[0];
				vecMax = m_LastPositionsList[0];

				// Obtentions des extrêmes des dernières position, ça donne une boîte dans laquelle le player est resté
				for(int i = 1; i < 20; i++)
				{
					vecMin.x = min(m_LastPositionsList[i].x, vecMin.x);
					vecMin.y = min(m_LastPositionsList[i].y, vecMin.y);
					vecMin.z = min(m_LastPositionsList[i].z, vecMin.z);

					vecMax.x = max(m_LastPositionsList[i].x, vecMax.x);
					vecMax.y = max(m_LastPositionsList[i].y, vecMax.y);
					vecMax.z = max(m_LastPositionsList[i].z, vecMax.z);
				}

				vecBox.x = abs(vecMax.x - vecMin.x);
				vecBox.y = abs(vecMax.y - vecMin.y);
				vecBox.z = abs(vecMax.z - vecMin.z);

				if(vecBox.x <= sv_autovocal_geolocalisation_distance.GetFloat() &&	vecBox.y <= sv_autovocal_geolocalisation_distance.GetFloat() &&	vecBox.z <= sv_autovocal_geolocalisation_distance.GetFloat())
				{
					PlayAutovocal(GEOLOCALISATION);
					m_LastPositionsList.RemoveAll(); // On est sûr que ça play pas toutes les secondes si le delay est court
				}
			}
		}
	}
...
ConVar mp_start_armor("mp_start_armor", "50", FCVAR_REPLICATED|FCVAR_NOTIFY, "Default armor value", true, 0, true, 200);
void CBasePlayer::Spawn( void )
{
	...
	if(HL2MPRules()->GetForceStartArmor() != -1)
		m_iArmorValue = HL2MPRules()->GetForceStartArmor();
	else
		m_iArmorValue = mp_start_armor.GetInt();

	if(HL2MPRules()->GetGameType() == GAME_PUSH) //No armor in push
		m_iArmorValue = 0;
 ...
 // only preserve the shadow flag
	...
	//Fov voulu par le joueur
	const char *szFov = engine->GetClientConVarValue( entindex(), "cl_fov" );
	int fov = atoi(szFov);
	SetFOV(this, fov-(fov/3));
	SetDefaultFOV(fov);
	...
	if(m_bIsFirstSpawn)
	{
		...
		if(random->RandomInt(0, 4) == 0)
			PlayAutovocal(NEWPLAYER);
		m_bIsFirstSpawn = false;
	}
	else if(m_nAutovocalAtRespawn != AUTOVOCAL_NONE)
	{
		PlayAutovocal(m_nAutovocalAtRespawn);
		m_nAutovocalAtRespawn = AUTOVOCAL_NONE;
	}

	Telefrag(&GetAbsOrigin());

	m_flLastUpdatePositionsListTime = gpGlobals->curtime;
	m_LastPositionsList.RemoveAll();
	m_LastPositionsList.AddToHead(GetAbsOrigin());

	m_flNextRespawn = FLT_MAX;
...
{
	Telefrag(newPosition);
	return BaseClass::Teleport(newPosition, newAngles, newVelocity);
}

void CBasePlayer::Telefrag(const Vector *position)
{
	trace_t tr;
	UTIL_TraceHull(*position, *position + Vector(1,1,1), VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, this, COLLISION_GROUP_PLAYER, &tr);
	while(tr.DidHitNonWorldEntity())
	{
		CBaseEntity *pEntity = tr.m_pEnt;
		if(pEntity == NULL)
			return;

		if(!(pEntity->IsNPC() || pEntity->IsPlayer()) || !pEntity->IsAlive())
			return;
	
		CBasePlayer *pPlayer = ToBasePlayer(pEntity);
		CBaseEntity *pInflictor = this;
		if(pPlayer != NULL){
			// Si on bute un coéquipier, mieux vaut que ça soit un suicide
			if((pPlayer->GetTeamNumber() == GetTeamNumber()) && (HL2MPRules()->IsTeamplay() || HL2MPRules()->GetGameType() == GAME_COOP))
				pInflictor = pPlayer;
		}

		CTakeDamageInfo info(pInflictor, pInflictor, pEntity->GetHealth() + 1, DMG_DISSOLVE | DMG_PREVENT_PHYSICS_FORCE /*DMG_NO_PHYSICS_FORCE*/);
		info.SetCustomKill(TELEFRAG);
...
void CBasePlayer::Precache( void )
{
	...
	//BP on précache les sons pour les autovocals
	int nAutovocals = AUTOVOCAL_MAX;
	for ( int i = 0; i < nAutovocals; ++i )
		PrecacheScriptSound( g_ppszAutovocals[i] );
...
void CBasePlayer::Respawn(bool killPlayer)
{
	if(killPlayer && m_lifeState == LIFE_ALIVE)
	{
		CommitSuicide();
		IncrementFragCount(1);
		
		if(HL2MPRules()->IsTeamplay() && HL2MPRules()->GetGameType() != GAME_FORTS && HL2MPRules()->GetGameType() != GAME_PUSH)
			g_Teams[GetTeamNumber()]->AddScore(1);
	}
	else
	{
		RemoveAllItems( true );
...
{ 
	m_iArmorValue += nCount;
	if (nMaxValue > 0)
		if (m_iArmorValue > nMaxValue)
			m_iArmorValue = nMaxValue;
}
...
//BP Changement de skin
void CBasePlayer::SetSkinNumberFromCvar(void)
{
	
	if(HL2MPRules()->IsTeamplay())
		return;

	const char *szSkin = engine->GetClientConVarValue( entindex(), "cl_skin" );
	float fskin = atof(szSkin);
	int skin = floor(fskin + 0.5f);
	int maxSkin = MAX_PLAYER_SKINS;

	if(Q_strcmp(szSkin, "ohyeahskinme") == 0)
	{
		skin = PLAYER_SKIN_DEV;
		maxSkin++;
	}

	if(skin <= 0 || skin > maxSkin)
		m_nSkin = GetRandomSkinNumber();
	else
		m_nSkin = toSkinId(skin);
}
void CBasePlayer::UpdateSkin(void)
{
	if(HL2MPRules()->IsTeamplay())
		SetSkinNumberFromTeam();
	else
		SetSkinNumberFromCvar();
}
void CBasePlayer::SetSkinNumberFromTeam(void)
{
	if(!HL2MPRules()->IsTeamplay())
		return;
...
int CBasePlayer::GetRandomSkinNumber(void)
{
	int retour = m_nSkin;
	while(retour == m_nSkin)
		retour = RandomInt(0, MAX_PLAYER_SKINS-1)*2;

	return retour;
}

// do I have any weapons at all?
...
{
	//BP empêche d'utiliser en mode spectateur
	if (GetTeamNumber() == TEAM_SPECTATOR)
		return;
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	else if ( stricmp( cmd, "spec_mode" ) == 0 ) // Change de mode spectateur
	{
		int mode;
		...
		//BP On switch entre OBS_MODE_CHASE et OBS_MODE_IN_EYE
	...
	else if ( stricmp( cmd, "jointeam" ) == 0 )
	{
		if( args.ArgC() < 2)
		{
			Msg("Usage : jointeam <0 ; 2 ; 3> (auto ; pink ; green)\n");
		...
		if (m_lifeState == LIFE_ALIVE)
		{
			if(HL2MPRules()->IsTeamplay())
			{
				...
				{
					Msg("Can't switch to your own team\n");
				...
				else if(team == 0)
				{
					...
					m_nAutovocalAtRespawn = RSACHANGETEAM;
				}
				else if(team == TEAM_GREEN || team == TEAM_PINK)
				{
					Respawn(true);
					ChangeTeam(team);
			...
			Msg("Can't change team while dead\n");

		return true;
	}
	else if ( stricmp( cmd, "swapteam" ) == 0 )
	{
		if ( m_fNextSuicideTime > gpGlobals->curtime )
			return true;
		if (m_lifeState == LIFE_ALIVE)
		{
			...
			if(HL2MPRules()->IsTeamplay())
			{
				if(GetTeamNumber() == TEAM_GREEN)
					ChangeTeam(TEAM_PINK);
				else if(GetTeamNumber() == TEAM_PINK)
					ChangeTeam(TEAM_GREEN);
				else
				{
					if(g_Teams[TEAM_GREEN]->GetNumPlayers() < g_Teams[TEAM_PINK]->GetNumPlayers())
...
// If HL2_DLL is defined, then baseflex.cpp already sends these.
		...
		SendPropFloat		( SENDINFO( m_flJumpSoundAvoidTime ), 0, SPROP_NOSCALE ),
...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_SERVERCLASS_ST( CBasePlayer, DT_BasePlayer )
		...
		SendPropInt		(SENDINFO(m_iArmorValue), 10 ),
...
void CBasePlayer::Taunt()
{
	/*SetAnimation(PLAYER_TAUNT);
	SetActivity(ACT_TAUNT);
	SetSequence(SelectHeaviestSequence(ACT_TAUNT));
	Msg("%i", SelectHeaviestSequence(ACT_TAUNT));*/

}
//BP méthodes concernant les autovocals
ConVar sv_autovocal_jumpsound_delay("sv_autovocal_jumpsound_delay", "3", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar sv_autovocal_delay("sv_autovocal_delay", "40", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar sv_autovocal_disable("sv_autovocal_disable", "0", FCVAR_REPLICATED | FCVAR_CHEAT);

bool CBasePlayer::PlayAutovocal(int nAutovocal)
{
	if(nAutovocal < 0 || nAutovocal > AUTOVOCAL_MAX)
	{
		Warning("CBasePlayer::PlayAutovocal : Invalid nAutovocal\n");

		return false;
	}

	if(CanPlayAutovocal(nAutovocal))
	{
		m_nAutovocalAtRespawn = AUTOVOCAL_NONE;
		m_nDelayedAutovocal = AUTOVOCAL_NONE;

		EmitSound(g_ppszAutovocals[nAutovocal]);

		m_flJumpSoundAvoidTime = gpGlobals->curtime + sv_autovocal_jumpsound_delay.GetFloat();
		m_flLastAutovocal = gpGlobals->curtime;
...
bool CBasePlayer::PlayAutovocal(int nAutovocal, float flDelay)
{
	if(nAutovocal < 0 || nAutovocal > AUTOVOCAL_MAX)
	{
		Warning("CBasePlayer::PlayAutovocal : Invalid nAutovocal\n");
	...
	if(CanPlayAutovocal(nAutovocal))
	{
		if(flDelay > 0)
		{
			m_nDelayedAutovocal = nAutovocal;
			m_flNextDelayedAutovocal = gpGlobals->curtime + flDelay;
...
bool CBasePlayer::CanPlayAutovocal(int nAutovocal)
{
	if(sv_autovocal_disable.GetBool())
		return false;

	if(m_lifeState != LIFE_ALIVE)
		return false;

	float delay = sv_autovocal_delay.GetFloat();

	if(nAutovocal == GEOLOCALISATION && (HL2MPRules()->GetGameType() == GAME_COOP || gpGlobals->eLoadType == MapLoad_Background )) // Pas de geoloc en coop ni en background
		return false;

	if(nAutovocal == RAMPAGE || nAutovocal == GENOCIDE) // Le rampage/slaughter peut se rejouer plus souvent, histoire de feel the power
		delay /= 3;

	if(nAutovocal == TAUNT_GENOCIDE) // BP, le jeu qui encourage les ragequits
		delay /= 3;

	if(nAutovocal == NEWPLAYER) // On le joue quoi qu'il arrive lui
		return true;
	
	return gpGlobals->curtime > m_flLastAutovocal + delay;
}
