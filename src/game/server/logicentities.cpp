...
void CLogicCase::InputPickRandom( inputdata_t &inputdata )
{
	...
	if ( nNumCases > 0 )
	{
		//int nRandom = (int)random->RandomFloat(0,nNumCases);
		int nRandom = BPRandom::GetInt(0, nNumCases); // Source's rand is always the same. Using our own pseudo pseudo PRNG
...
void CLogicCase::InputPickRandomShuffle( inputdata_t &inputdata )
{
	...
	if ( nCaseCount > 0 )
	{
		int nRandom = BPRandom::GetInt(0, nCaseCount); // Source's rand is always the same. Using our own pseudo pseudo PRNG
