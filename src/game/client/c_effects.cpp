...
ConVar r_RainSplashPercentage( "r_RainSplashPercentage", "12", FCVAR_CHEAT ); // N% chance of a rain particle making a splash.
...
ConVar r_RainSideVel( "r_RainSideVel", "250", FCVAR_CHEAT, "How much sideways velocity rain gets." );
...
ConVar CClient_Precipitation::s_raindensity( "r_raindensity","0.002", FCVAR_CHEAT);
ConVar CClient_Precipitation::s_rainwidth( "r_rainwidth", "0.4", FCVAR_CHEAT );
ConVar CClient_Precipitation::s_rainlength( "r_rainlength", "0.05f", FCVAR_CHEAT );
ConVar CClient_Precipitation::s_rainspeed( "r_rainspeed", "1400.0f", FCVAR_CHEAT );
...
//
//-----------------------------------------------------------------------------
ConVar r_enviromentaleffects( "r_enviromentaleffects","1");
inline bool CClient_Precipitation::SimulateRain( CPrecipitationParticle* pParticle, float dt )
{
	...
	if(r_enviromentaleffects.GetBool())
	{
		trace_t trace;
		...
		{
			if ( RandomInt( 0, 100 ) <= r_RainSplashPercentage.GetInt() )
				DispatchParticleEffect( "Rain_Splash2", trace.endpos,trace.m_pEnt->GetAbsAngles() , NULL );
