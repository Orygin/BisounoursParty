...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_RECV_TABLE_NOBASE( C_BasePlayer, DT_LocalPlayerExclusive )
		...
		RecvPropFloat		( RECVINFO ( m_flJumpSoundAvoidTime ) ),
...
// -------------------------------------------------------------------------------- //
	IMPLEMENT_CLIENTCLASS_DT(C_BasePlayer, DT_BasePlayer, CBasePlayer)
		...
		RecvPropInt		(RECVINFO(m_iArmorValue)), //BP
...
BEGIN_PREDICTION_DATA( C_BasePlayer )
	...
	DEFINE_PRED_FIELD( m_iArmorValue, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
...
//BP Taunt
void C_BasePlayer::Taunt()
{
	/*SetAnimation(PLAYER_TAUNT);
	SetSequence(SelectWeightedSequence(ACT_TAUNT));
	Msg("%i", SelectWeightedSequence(ACT_TAUNT));*/
	//Msg("Hey, taunting here \n");
}
void C_BasePlayer::SetHealth(int health)
{
...
bool C_BasePlayer::Init(int entnum, int serialnum)
{
	Precache();
	return BaseClass::Init(entnum, serialnum);
}
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
#define BLOOM_DECAY 0.8f
ConVar Bloom_Hurt("bp_bloom_enable", "1");
void C_BasePlayer::PostThink( void )
{
	...
	if ( IsAlive())
	{
		...
		StudioFrameAdvance();
		if(m_fStartBloomDecay > gpGlobals->curtime)
		{
			cvar->FindVar("mat_bloom_scalefactor_scalar")->SetValue((m_fStartBloomDecay-gpGlobals->curtime)/m_fBloomDecay*10);
		}
		else
			cvar->FindVar("mat_bloom_scalefactor_scalar")->SetValue(0);

		if(m_iOldHealth<m_iHealth && Bloom_Hurt.GetBool())
		{
			cvar->FindVar("mat_bloom_scalefactor_scalar")->SetValue(m_iHealth-m_iOldHealth);
			m_fBloomDecay = ((m_iHealth-m_iOldHealth)*0.04f);
			m_fStartBloomDecay = gpGlobals->curtime+m_fBloomDecay;
		}
