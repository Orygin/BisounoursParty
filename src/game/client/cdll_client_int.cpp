...
#if GAME_BETA
#define NEBULEUSE_GAMEVERSION 5114
#else
#define NEBULEUSE_GAMEVERSION 5102
#endif 
...
bp_achievListener *g_AchListener = NULL;
...
static ConVar bp_langFrench("bp_langFrench", "0", FCVAR_ARCHIVE, "Language of the autovocals");
static ConVar bpNebuleuseEnabled("bp_nebuleuse_enabled", "1", FCVAR_ARCHIVE, "Enable or not nebuleuse (require restart)");
static ConVar bpNebuleuseHost("bp_nebuleuse_host", "http://www.bisounoursparty.com/nebuleuse", FCVAR_ARCHIVE, "Host of the game network (require restart)");
...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
	...
	{	
		int GameVersion = NEBULEUSE_GAMEVERSION;
		bool DevChannel = (CommandLine()->FindParm("-nebdev") || GAME_BETA);

		if(filesystem->FileExists("version", "GAMEBIN")) // version file overwrites the code
		{
			FileHandle_t fh = filesystem->Open("version", "r", "GAMEBIN");
			if(fh)
			{ // This is ugly, but i'm too lazy atm to make a better solution
				int file_len = filesystem->Size(fh);
				char* VersionInfo = new char[file_len + 1];
 
				filesystem->Read(VersionInfo, file_len,fh);
				VersionInfo[file_len] = 0;

				GameVersion = VersionInfo[0]*1000 +VersionInfo[1]*100 +VersionInfo[2]*10 +VersionInfo[3];
 
				if(file_len >= 7)
				{
					char* ChannelInfo = new char[4];
					ChannelInfo[0] = VersionInfo[5];
					ChannelInfo[1] = VersionInfo[6];
					ChannelInfo[2] = VersionInfo[7];
					ChannelInfo[3] = 0;

					if(!strcmp(ChannelInfo, "dev"))
						DevChannel = true;
					else
						DevChannel = false;
		...
		if(DevChannel)
			host = "http://www.bisounoursparty.com/NebuleuseDev";
		else
			host = CommandLine()->ParmValue("-nebuleuse_host", "http://www.bisounoursparty.com/nebuleuse");

		Nebuleuse *neb = new Nebuleuse(steamapicontext->SteamUser()->GetSteamID().ConvertToUint64(), host, GameVersion, NEBULEUSE_GAMEKEY);
		neb->SetLogCallBack(&NebuleuseLogCb);
	}
...
{
	if(g_pCVar->FindVar( "bp_nebuleuse_enabled" )->GetInt() == 1 && g_Nebuleuse)
		g_Nebuleuse->Init();

	g_AchListener = new bp_achievListener();
...
{
	//Ninja translate if needed
	if( bp_langFrench.GetBool() && filesystem->FileExists("sound/player/autovocals_french") )
	{
		filesystem->RenameFile("sound/player/autovocals","sound/player/autovocals_english");
		filesystem->RenameFile("sound/player/autovocals_french","sound/player/autovocals");
	}
	else if( !bp_langFrench.GetBool() && filesystem->FileExists("sound/player/autovocals_english") )
	{
		filesystem->RenameFile("sound/player/autovocals","sound/player/autovocals_french");
		filesystem->RenameFile("sound/player/autovocals_english","sound/player/autovocals");
	}
...
void CHLClient::LevelInitPreEntity( char const* pMapName )
{
	...
	g_Nebuleuse->SetCurrentMap(pMapName);
}
...
void CHLClient::LevelShutdown( void )
{
	...
	if(g_Nebuleuse)
	{
		PlayerStat st = g_Nebuleuse->GetPlayerStats("timePlayed");
		st.Value += (int) (gpGlobals->curtime - m_flMapStartTime);
		g_Nebuleuse->SetPlayerStats(st);
		g_Nebuleuse->SendPlayerStats();
	}
}
