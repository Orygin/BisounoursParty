...
void CHudNumericDisplay::PaintNumbers(HFont font, int xpos, int ypos, int value)
{
	...
	Color c(0, 0, 0, 255);
	//BP Contours
	int xposOutline = xpos - (UTIL_ComputeStringWidth(m_hNumberOutlineFont, unicode) - UTIL_ComputeStringWidth(m_hNumberFont, unicode)) / 2;
	surface()->DrawSetTextFont(m_hNumberOutlineFont);
	surface()->DrawSetTextColor(c);
	surface()->DrawSetTextPos(xposOutline, ypos - 3);
