...
#define MAX_ID_STRING 256
	...
	if ( iEntIndex )
	{
		...
		if ( IsPlayerIndex( iEntIndex ) )
		{
			...
			if ( ( HL2MPRules()->IsTeamplay() == true && pPlayer->InSameTeam(pLocalPlayer) ) || HL2MPRules()->GetGameType() == GAME_COOP )
			{
			...
			if ( bShowHealth )
			{
				_snwprintf( wszArmorText, ARRAYSIZE(wszArmorText) - 1, L"%.0f",  (float) pPlayer->GetArmor() );
				wszHealthText[ ARRAYSIZE(wszArmorText)-1 ] = '\0';
		...
		{
			if ( bShowPlayerName && bShowHealth && bShowArmor)
			{
