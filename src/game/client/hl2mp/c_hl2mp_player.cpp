...
IMPLEMENT_CLIENTCLASS_DT(C_HL2MP_Player, DT_HL2MP_Player, CHL2MP_Player)
	...
	RecvPropEHandle( RECVINFO( m_hKiller ) ), //BP tueur du joueur, utile pour le suivi du regard
...
static ConVar cl_skin("cl_skin", "0", FCVAR_ARCHIVE | FCVAR_USERINFO, "Player Skin");
static ConVar cl_fov("cl_fov", "100", FCVAR_ARCHIVE | FCVAR_USERINFO, "Field Of View", true, 75, true, 120);
static ConVar cl_defaultweapon( "cl_defaultweapon", "weapon_uzi", FCVAR_USERINFO | FCVAR_ARCHIVE, "Default Spawn Weapon");
...
bool C_HL2MP_Player::ShouldPredict()
{
	return true;
}
//-----------------------------------------------------------------------------
...
void C_HL2MP_Player::CalcView( Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar, float &fov )
{
	...
	{
		//BP Ragdoll à la première personne
		if ( m_hRagdoll.Get() )
		{
			CBaseEntity* pKiller = GetKiller();

			C_HL2MPRagdoll *pRagdoll = (C_HL2MPRagdoll*)m_hRagdoll.Get();
			/*if(!pKiller->IsPlayer())
				pRagdoll->GetAttachment( pRagdoll->LookupAttachment("first"), eyeOrigin, eyeAngles );*/
			

			//BP on suit le tueur du regard

			
			if(pKiller != NULL && pKiller != this && pKiller->IsAlive()){
				Vector distance = pKiller->GetAbsOrigin() - GetAbsOrigin();
				VectorNormalize(distance);
				VectorAngles(distance, eyeAngles);
				eyeAngles.y += 5;
				eyeAngles.x += 5;
				pRagdoll->m_nSkin = this->m_nSkin;
			}
			else if (pKiller == this || pKiller == NULL)
				pRagdoll->GetAttachment( pRagdoll->LookupAttachment("first"), eyeOrigin, eyeAngles );
...
CBaseEntity* C_HL2MP_Player::GetKiller(){
	CBaseEntity* pKiller = m_hKiller;
...
//DevMsg("pPlayer->m_nSkin %d\n",pPlayer->m_nSkin);
//DevMsg("this->m_nSkin %d\n",this->m_nSkin);
//	int skin = atoi(cl_skin.GetString());
//	if(skin == 0)
	//{
		//skin = 1;
	//}
	...
	if(pPlayer)
		Skin = pPlayer->m_nSkin;

	InitAsClientRagdoll( boneDelta0, boneDelta1, currentBones, boneDt, Skin );
	//DevMsg("C_Player->m_nSkin %d\n",this->m_nSkin);
}
