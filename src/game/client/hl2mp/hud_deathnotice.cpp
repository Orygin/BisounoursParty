...
private:
	...
	CPanelAnimationVar( vgui::HFont, m_hTextFont, "TextFont", "HudDeathNoticeFont" );
...
{
	m_iconD_skull = gHUD.GetIcon("death_skull");
...
void CHudDeathNotice::Paint()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		bool pedoKill = !strcmp(m_DeathNotices[i].Killer.szName, "pedobear");
		...
		if(m_DeathNotices[i].Victim.iEntIndex == 0)
			g_pVGuiLocalize->ConvertANSIToUnicode( "", victim, sizeof( victim ) );
		else
		...
		if(pedoKill)
		{
			wcscpy(killer, victim); // The victim name is first
			wcscpy(victim, g_pVGuiLocalize->Find("#hud_rapedbypedo")); // The victim is the 'raped by' text
		}

		// Get the local position for this notice
		int iconWide = surface()->GetCharacterWidth(icon->hFont, icon->cCharacterInFont);
		int iconTall = surface()->GetFontTall( icon->hFont);
		int length = m_DeathNotices[i].iSuicide ? iconWide + UTIL_ComputeStringWidth(m_hTextFont, victim) : UTIL_ComputeStringWidth(m_hTextFont, victim) + iconWide + UTIL_ComputeStringWidth(m_hTextFont, killer);
		int x = (ScreenWidth() - length) / 2;
		int y = 10 + (m_flLineHeight * i);

		// Only draw killers name if it wasn't a suicide
		if ( !m_DeathNotices[i].iSuicide || pedoKill )
		{
		...
		// Draw death weapon only if not pedobear
		if(!pedoKill)
		{
			icon->DrawSelf(x, y, iconWide, iconTall, COLOR_WHITE);
			x += iconWide;
		}
...
void CHudDeathNotice::FireGameEvent( IGameEvent * event )
{
	...
	if(!strcmp(killedwith, "pedobear"))
		killer_name = "pedobear";
	...
	deathMsg.flDisplayTime = gpGlobals->curtime + hud_deathnotice_time.GetInt();
