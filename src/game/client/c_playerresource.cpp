...
const char *C_PlayerResource::GetPlayerName( int iIndex )
{
	...
	{
		//BP WTF? Commenté pour éviter les messages de debug inutiles
		//Assert( false );
		return "ERRORNAME";
	}
...
int C_PlayerResource::GetTeam(int iIndex )
{
	if ( iIndex < 1 || iIndex > MAX_PLAYERS )
	{
		//BP WTF? Commenté pour éviter les messages de debug
		//Assert( false );
		return 0;
	}
	else
...
int C_PlayerResource::GetHighestScore()
{
	int score = 0;
	for(int i=0; i < (sizeof(m_iScore) / sizeof(m_iScore[0])); i++)
	{
		if(m_iScore[i] > score)
			score = m_iScore[i];
	}
