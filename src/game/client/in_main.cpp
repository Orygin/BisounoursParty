...
//BP Taunt
void IN_TauntUp( const CCommand &args ) { KeyUp( &in_taunt, args[1] ); }
void IN_TauntDown( const CCommand &args ) { KeyDown( &in_taunt, args[1] ); }
...
int CInput::GetButtonBits( int bResetState )
{
	...
	//BP Taunt
//	CalcButtonBits( bits, IN_TAUNT, s_ClearInputState, &in_taunt, bResetState );
...
//BP Taunt
static ConCommand endtaunt( "-taunt", IN_TauntUp );
static ConCommand startaunt( "+taunt", IN_TauntDown );
#ifdef TF_CLIENT_DLL
