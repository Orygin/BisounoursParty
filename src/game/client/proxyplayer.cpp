...
//BP Proxy pour BP à partir d'ici

extern ConVar cl_skin;

class CPlayerSkinProxy : public CResultProxy
{
public:
	virtual void OnBind( void *pC_BaseEntity );
};

void CPlayerSkinProxy::OnBind( void *pC_BaseEntity )
{	
	C_BasePlayer* pPlayer = C_BasePlayer::GetLocalPlayer();
	if (!pPlayer)
		return;

	if(pPlayer->m_nSkin == 16) // Bonus rose
	{
		m_pResult->SetIntValue(8);
		return;
	}
	else if(pPlayer->m_nSkin == 17) // Bonus vert
	{
		m_pResult->SetIntValue(9);
		return;
	}

	if(pPlayer->m_nSkin % 2 != 0) // Si impair : skin de mort : pas bien
		m_pResult->SetIntValue( (pPlayer->m_nSkin - 1) / 2 );
	else
		m_pResult->SetIntValue( pPlayer->m_nSkin  / 2 );
}
...
class CPlayerFortsTimerProxy : public CResultProxy
{
public:
	virtual void OnBind( void *pC_BaseEntity );
	bool Init( IMaterial *pMaterial, KeyValues *pKeyValues );
...
EXPOSE_INTERFACE( CPlayerFortsTimerProxy, IMaterialProxy, "PlayerFortsTimerProxy" IMATERIAL_PROXY_INTERFACE_VERSION );
