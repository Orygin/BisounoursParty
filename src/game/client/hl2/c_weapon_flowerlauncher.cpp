// BISOUNOURS PARTY : FLOWERLAUNCHER (client)
...
class C_FlowerBolt : public C_BaseCombatCharacter
{
	DECLARE_CLASS( C_FlowerBolt, C_BaseCombatCharacter );
	DECLARE_CLIENTCLASS();
public:
...
private:
	...
	C_FlowerBolt( const C_FlowerBolt & ); // not defined, not accessible

	Vector	m_vecLastOrigin;
	bool	m_bUpdated;
};

IMPLEMENT_CLIENTCLASS_DT( C_FlowerBolt, DT_FlowerBolt, CFlowerBolt )
END_RECV_TABLE()

C_FlowerBolt::C_FlowerBolt( void )
{
}

void C_FlowerBolt::OnDataChanged( DataUpdateType_t updateType )
{
...
int C_FlowerBolt::DrawModel( int flags )
{
...
void C_FlowerBolt::ClientThink( void )
{
	m_bUpdated = false;
}