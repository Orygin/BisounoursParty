...
//BP Proxy pour le changement de l'image à l'écran de chargement
class CTimedRandomProxy : public CResultProxy
{
public:
	CTimedRandomProxy(void);
	virtual bool Init( IMaterial *pMaterial, KeyValues *pKeyValues );
	virtual void OnBind( void *pC_BaseEntity );

private:
	CFloatInput m_RangeMin;
	CFloatInput m_RangeMax;
	CFloatInput m_ChangeTime;
	float m_flLastChangeTime;
	int m_iLastValue;
};

CTimedRandomProxy::CTimedRandomProxy(void)
{
	m_flLastChangeTime = 0;
	m_iLastValue = 0;
}

bool CTimedRandomProxy::Init( IMaterial *pMaterial, KeyValues *pKeyValues )
{
	if (!CResultProxy::Init( pMaterial, pKeyValues ))
		return false;

	if (!m_RangeMin.Init( pMaterial, pKeyValues, "rangeMin", 0 ))
		return false;
	if (!m_RangeMax.Init( pMaterial, pKeyValues, "rangeMax", 1 ))
		return false;
	if (!m_ChangeTime.Init( pMaterial, pKeyValues, "changeTime", 30.0f ))
		return false;

	return true;
}

void CTimedRandomProxy::OnBind( void *pC_BaseEntity )
{
	int rangeMin = (int)m_RangeMin.GetFloat();
	int rangeMax = (int)m_RangeMax.GetFloat();

	if(rangeMin == rangeMax)
		return;

	float changeTime = abs(m_ChangeTime.GetFloat());
	if(gpGlobals->curtime > m_flLastChangeTime)
	{
		m_flLastChangeTime = gpGlobals->curtime + changeTime;
		int randomInt = m_iLastValue;
		while(randomInt == m_iLastValue)
		{
			randomInt = random->RandomInt(rangeMin, rangeMax);
		}
		m_pResult->SetIntValue( randomInt );
	}
}

EXPOSE_INTERFACE( CTimedRandomProxy, IMaterialProxy, "TimedRandom" IMATERIAL_PROXY_INTERFACE_VERSION );