...
#define ACHIEVEMENT_NOTIFICATION_DURATION 4.5f

void AchievementEarn(int index)
{
	...
	if ( pPanel )
	{		
		pPanel->AddNotification( "HL2_KILL_ODESSAGUNSHIP", g_pVGuiLocalize->Find( "#bp_AchievementEarned" ), ACHIEVEMENT_LOCALIZED_NAME_FROM_STR(ach.Name) );
	}
}
//-----------------------------------------------------------------------------
// Purpose: 
...
void CAchievementNotificationPanel::Init()
{
	...
	g_Nebuleuse->SetAchievementCallBack(&AchievementEarn);
}
...
void CAchievementNotificationPanel::ShowNextNotification()
{
	...
	int iPanelX = XRES(625) - iPanelWidth;
...
CON_COMMAND_F( achievement_notification_test, "Test the hud notification UI", FCVAR_CHEAT )
{
	...
	*/
	...
	enginesound->EmitSound(	filter, SOUND_FROM_WORLD, CHAN_STATIC, "gameplay/achievement_earned.wav", 1.0f, 0.2, 0, 100, &C_BasePlayer::GetLocalPlayer()->GetAbsOrigin()); // Play a sound where the player is
#if 0
